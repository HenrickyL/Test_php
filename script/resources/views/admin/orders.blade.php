    ...
    <form action="{{ route('admin.post.searchOrders') }}" method="GET">
        <div class="form-group form-group-feedback form-group-feedback-right search-box">
            <input type="text" class="form-control form-control-lg search-input"
                placeholder="Procurar pedido pelo ID..." name="query">
                        ...
                        <tr>
                                ...
                                @if(config("settings.restaurantAcceptTimeThreshold") != NULL)
                                    @if ($order->orderstatus_id == 1)
                                        @if($order->created_at->diffInMinutes(\Carbon\Carbon::now()) >= (int) config("settings.restaurantAcceptTimeThreshold"))
                                            <span class="pulse pulse-warning" data-popup="tooltip" title="" data-placement="bottom" data-original-title="Pedido não aceito pela loja. Tarde por {{ $order->created_at->diffInMinutes(\Carbon\Carbon::now()) - 5 }} mins."></span>
                                        @endif
                                    @endif
                                @endif
                                @if(config("settings.deliveryAcceptTimeThreshold") != NULL)
                                     @if ($order->orderstatus_id == 2 && $order->delivery_type == 1) 
                                       @if($order->created_at->diffInMinutes(\Carbon\Carbon::now()) >= (int) config("settings.deliveryAcceptTimeThreshold"))
                                            <span class="pulse pulse-danger" data-popup="tooltip" title="" data-placement="bottom" data-original-title="Pedido não aceito pelo entregador. Tarde por {{ $order->created_at->diffInMinutes(\Carbon\Carbon::now()) - 5 }} mins."></span>
                                        @endif
                                    @endif
                                @endif
                            </td>
                            ...
                            <td>
                                <span class="badge badge-flat border-grey-800 text-default text-capitalize text-left">
                                    ...
                                    @if ($order->orderstatus_id == 4) Retirar @endif
                                    @if ($order->orderstatus_id == 5) Completado @endif
                                    @if ($order->orderstatus_id == 6) Cancelado @endif
                                    @if ($order->orderstatus_id == 7) Pronto para Retirar @endif
