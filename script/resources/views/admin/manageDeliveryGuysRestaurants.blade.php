...
@section("title") Entregador - Painel
@endsection
@section('content')
<style>
        ...
        <div class="card">
            <div class="card-body">
                <legend class="font-weight-semibold text-uppercase font-size-sm">
                    <i class="icon-address-book mr-2"></i> Entregadores Lojas
                </legend>
                <div class="form-group row form-group-feedback form-group-feedback-right">
                    @if(count($userRestaurants) === 0)
                    <div class="col-lg-9">
                        <p class="text-warning">{{ $user->name }} nao foi atribuido a nenhuma loja.</p>
                    </div>
                    @else
                    <br>
                    <div class="col-lg-9">
                        <p><strong>{{ $user->name }}</strong> entrega para a loja <strong>{{ $userRestaurants->count() }} </strong>.</p>
                ...
                @if(!$user->delivery_guy_detail)
                    <p>Todos os detalhes para {{ $user->name }} não são preenchidos. Preencha todos os detalhes obrigatórios para atribuir lojas a {{ $user->name }}</p>
                    <strong><a href="{{ route('admin.get.editUser', $user->id) }}#deliveryGuyDetails">Clique aqui </a></strong> editar os detalhes
                @else
                ...
                <div class="col-lg-12">
                    <form action="{{ route('admin.updateDeliveryGuysRestaurants') }}" method="POST">
                        ...
                        <div class="text-right mb-4">
                            <button type="button" class="btn btn-primary btn-labeled btn-labeled-left btn-sm" id="checkAll" data-popup="tooltip" title="Duplo clique para marcar" data-placement="left">
                            <b><i class="icon-check ml-1"></i></b>
                                Marcar todos
                            </button>
                            <button type="button" class="btn btn-primary btn-labeled btn-labeled-left btn-sm" id="unCheckAll" data-popup="tooltip" title="Duplo clique para desmarcar" data-placement="top">
